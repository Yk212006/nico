...
def build_pi_settings() -> Settings:
    return Settings(
        ...
        default_provider=os.getenv("NICO_DEFAULT_PROVIDER", "ollama"),
        log_level=os.getenv("NICO_LOG_LEVEL", "INFO"),
        enable_tools=os.getenv("NICO_ENABLE_TOOLS", "false").lower() == "true",
        enable_memory=os.getenv("NICO_ENABLE_MEMORY", "false").lower() == "true",
        enable_voice=os.getenv("NICO_ENABLE_VOICE", "false").lower() == "true",
    )
